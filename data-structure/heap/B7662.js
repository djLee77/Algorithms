...
rl.on('line', (line) => {
  ...
  if (cmd === 'I') {
    ...
    if (actualQuantity.get(value) > 0) {
      actualQuantity.set(value, actualQuantity.get(value) + 1);
    } else {
      actualQuantity.set(value, 1);
    }
  }

  if (cmd === 'D' && value === 1) {
    let max = maxHeap.pop();
    while (maxHeap.length() > 0 && actualQuantity.get(max) < 1) {
      max = maxHeap.pop();
    }
    actualQuantity.set(max, actualQuantity.get(max) - 1);
  }

  if (cmd === 'D' && value === -1) {
    let min = minHeap.pop();
    while (minHeap.length() > 0 && actualQuantity.get(min) < 1) {
      min = minHeap.pop();
    }
    actualQuantity.set(min, actualQuantity.get(min) - 1);
  }
